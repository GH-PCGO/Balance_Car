...
//usart配置
void usart_gpio_dma_config(uint32_t band_rate){
	//1.开启时钟
...
void BSP_USART_DMA_IRQHandler(void){
	...
	if(usart_interrupt_flag_get(BSP_USART,USART_INT_FLAG_IDLE) == SET)												 // 检测到帧中断
	{
		...
		/* 处理DMA接收到的数据 */
		g1_recv_length = USART_RECEIVE_LENGTH - dma_transfer_number_get(BSP_DMA,BSP_DMA_CHANNEL);	 // 计算实际接收的数据长度
		//g1_recv_buff[g1_recv_length] = '\0';																											 // 数据接收完毕，数组结束标志
																																		
	
		
		g1_recv_complete_flag = 1;  // 接收完成 
...
//发送一个字符
void usart_dma_send_data(uint8_t ucch){
	usart_data_transmit(BSP_USART,(uint8_t)ucch);							 // 发送数据
	while(RESET == usart_flag_get(BSP_USART,USART_FLAG_TBE));  // 等待发送数据缓冲区标志置位
}
//发送字符串
void usart_dma_send_string(uint8_t *ucstr){
	while(ucstr && *ucstr){        		// 地址为空或者值为空跳出
	  usart_dma_send_data(*ucstr++);  // 发送单个字符
	}
}

//重写fputc
int fputc(int ch, FILE *f){
     usart_dma_send_data(ch);
     // 等待发送数据缓冲区标志置位
     return ch;
}